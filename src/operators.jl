...
# Number--VariableRef
(+)(lhs::Number, rhs::VariableRef) = AffExpr([rhs],[+1.],convert(Float64,lhs))
(-)(lhs::Number, rhs::VariableRef) = AffExpr([rhs],[-1.],convert(Float64,lhs))
(*)(lhs::Number, rhs::VariableRef) = AffExpr([rhs],[convert(Float64,lhs)], 0.)
...
(-)(lhs::VariableRef) = AffExpr([lhs],[-1.0],0.0)
(*)(lhs::AbstractJuMPScalar) = lhs # make this more generic so extensions don't have to define unary multiplication for our macros
# VariableRef--Number
(+)(lhs::VariableRef, rhs::Number) = (+)( rhs,lhs)
(-)(lhs::VariableRef, rhs::Number) = (+)(-rhs,lhs)
(*)(lhs::VariableRef, rhs::Number) = (*)(rhs,lhs)
(/)(lhs::VariableRef, rhs::Number) = (*)(1./rhs,lhs)
# VariableRef--VariableRef
(+)(lhs::VariableRef, rhs::VariableRef) = AffExpr([lhs,rhs], [1.,+1.], 0.)
(-)(lhs::VariableRef, rhs::VariableRef) = AffExpr([lhs,rhs], [1.,-1.], 0.)
(*)(lhs::VariableRef, rhs::VariableRef) = QuadExpr([lhs],[rhs],[1.],AffExpr(VariableRef[],Float64[],0.))
# VariableRef--AffExpr
(+)(lhs::V, rhs::GenericAffExpr{C,V}) where {C,V<:JuMPTypes} =
    GenericAffExpr{C,V}(vcat(lhs,rhs.vars),vcat(one(C),rhs.coeffs), rhs.constant)
(-)(lhs::V, rhs::GenericAffExpr{C,V}) where {C,V<:JuMPTypes} =
    GenericAffExpr{C,V}(vcat(lhs,rhs.vars),vcat(one(C),-rhs.coeffs),-rhs.constant)
function (*)(lhs::VariableRef, rhs::AffExpr)
    n = length(rhs.vars)
    if rhs.constant != 0.
...
(/)(lhs::VariableRef, rhs::AffExpr) = error("Cannot divide a variable by an affine expression")
# VariableRef--QuadExpr
(+)(v::VariableRef, q::QuadExpr) = QuadExpr(copy(q.qvars1),copy(q.qvars2),copy(q.qcoeffs),v+q.aff)
(-)(v::VariableRef, q::QuadExpr) = QuadExpr(copy(q.qvars1),copy(q.qvars2),    -q.qcoeffs ,v-q.aff)
...
function (^)(lhs::Union{VariableRef,AffExpr}, rhs::Integer)
    if rhs == 2
        return lhs*lhs
    elseif rhs == 1
        return QuadExpr(lhs)
...
(^)(lhs::Union{VariableRef,AffExpr}, rhs::Number) = error("Only exponents of 0, 1, or 2 are currently supported. Are you trying to build a nonlinear problem? Make sure you use @NLconstraint/@NLobjective.")
# AffExpr--VariableRef
...
function (*)(lhs::AffExpr, rhs::VariableRef)
    n = length(lhs.vars)
    if lhs.constant != 0.
...
(/)(lhs::AffExpr, rhs::VariableRef) = error("Cannot divide affine expression by a variable")
# AffExpr--AffExpr
...
function (*)(lhs::AffExpr, rhs::AffExpr)
    ret = QuadExpr(VariableRef[],VariableRef[],Float64[],AffExpr(VariableRef[],Float64[],0.))
...
# QuadExpr--VariableRef
(+)(q::QuadExpr, v::VariableRef) = (+)(v,q)
(-)(q::QuadExpr, v::VariableRef) = QuadExpr(copy(q.qvars1),copy(q.qvars2),copy(q.qcoeffs),q.aff-v)
(*)(q::QuadExpr, v::VariableRef) = error("Cannot multiply a quadratic expression by a variable")
(/)(q::QuadExpr, v::VariableRef) = error("Cannot divide a quadratic expression by a variable")
# QuadExpr--AffExpr
...
Base.sum(j::Array{VariableRef}) = AffExpr(vec(j), ones(length(j)), 0.0)
Base.sum(j::AbstractArray{VariableRef}) = sum([j[i] for i in eachindex(j)]) # to handle non-one-indexed arrays.
...
Base.promote_rule(::Type{VariableRef},::Type{R}       ) where {R<:Real} = AffExpr
Base.promote_rule(         ::Type{VariableRef},::Type{AffExpr} ) = AffExpr
Base.promote_rule(         ::Type{VariableRef},::Type{QuadExpr}) = QuadExpr
...
# Special-case because the the base version wants to do fill!(::Array{VariableRef}, zero(AffExpr))
function Base.diagm(x::AbstractVector{VariableRef})
...
for (func,_) in Calculus.symbolic_derivatives_1arg(), typ in [:VariableRef,:AffExpr,:QuadExpr]
...
*(::T,::S) where {T<:QuadExpr,S<:Union{VariableRef,AffExpr,QuadExpr}} =
    error( "*(::$T,::$S) is not defined. $op_hint")
(*)(lhs::QuadExpr, rhs::QuadExpr) =
    error( "*(::QuadExpr,::QuadExpr) is not defined. $op_hint")
*(::S,::T) where {T<:QuadExpr,S<:Union{VariableRef,AffExpr,QuadExpr}} =
    error( "*(::$S,::$T) is not defined. $op_hint")
/(::S,::T) where {S<:Union{Number,VariableRef,AffExpr,QuadExpr},T<:Union{VariableRef,AffExpr,QuadExpr}} =
    error( "/(::$S,::$T) is not defined. $op_hint")
